...
def main():
	...
	for idx, imgdir in enumerate(imgdirlist):
		...
		pResTemp, pSumTemp, pRois = measure_mask(tRes, tSum, os.path.join(imgdir, bases[0]), os.path.join(imgdir, bases[1]), bound)
		aggRes = copy_table(pResTemp, tRes, imgName, 'punctate')
		aggSum = copy_table(pSumTemp, tSum, imgName, 'punctate')
		
		dResTemp, dSumTemp, dRois = measure_mask(tRes, tSum, os.path.join(imgdir, bases[0]), os.path.join(imgdir, bases[2]), bound)
		aggRes = copy_table(dResTemp, tRes, imgName, 'diffuse')
		aggSum = copy_table(dSumTemp, tSum, imgName, 'diffuse')
	...
	tRes.save(os.path.join(str(output_dir), "roi_data.csv"))
	tSum.save(os.path.join(str(output_dir), "summary_data.csv"))
...
def measure_mask(aggRes, aggSum, imagepath, maskpath, boundaries):
	...
	this_calibration = image.getCalibration()
	mask = IJ.openImage(maskpath)
	mask.setCalibration(this_calibration)
	
	outer, inner = get_soma_measurements(mask, boundaries)
	
	if image.width != mask.width or image.height != mask.height:
		print("WARNING: Image " + dirname + " masks have incorrect dimensions!")
	...
	# several predictions with several input patches, meaning each pixel is predicted on multiple times
	...
	# minimum area of 0.03 is ~= surface area of particle with diameter 200nm (best Airyscan resolution)
	pixelArea = this_calibration.pixelWidth ** 2
	pa = ParticleAnalyzer(options, measurements, rT, 0.03 / pixelArea, 10000000, 0, 1)
...
# pre-conditions:
# post-conditions:
def copy_table(sourceTable, targetTable, sourceName = "", roiType = ""):
	...
	for i in range(sourceTable.getCounter()):
		...
		if roiType != "":
			targetTable.setValue("RoiType", targetCount + i, roiType)
		for col in cols:
